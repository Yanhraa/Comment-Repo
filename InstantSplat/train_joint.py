...
def save_pose(path, quat_pose, train_cams, llffhold=2):
    """
    将基于四元数的相机位姿转换为适合保存的格式，并保存到文件中。

    参数:
    - path (str): 要保存位姿的文件路径。
    - quat_pose (list of torch.Tensor): 基于四元数的相机位姿列表。
    - train_cams (list of Camera): 包含 `colmap_id` 属性的相机对象列表。
    - llffhold (int, optional): 在此函数中未使用的参数。默认值为2。
    """

    output_poses=[]
    index_colmap = [cam.colmap_id for cam in train_cams]
    for quat_t in quat_pose:
        w2c = get_camera_from_tensor(quat_t) # 将四元数转换为相机矩阵
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    # 初始化
    first_iter = 0 # 记录第一次迭代的索引
    tb_writer = prepare_output_and_logger(dataset)  # 准备TensorBoard日志记录器
    gaussians = GaussianModel(dataset.sh_degree)  # 初始化高斯模型
    scene = Scene(dataset, gaussians, opt=args, shuffle=True)  # 设置训练场景                                                                      
    gaussians.training_setup(opt)  # 准备高斯模型的训练设置
    if checkpoint:
        # 从检查点加载模型参数
    ...
    viewpoint_stack = None # 初始化视角堆栈
    ema_loss_for_log = 0.0 # 指数加权移动平均损失
    progress_bar = tqdm(range(first_iter, opt.iterations), desc="Training progress")
    first_iter += 1

    start = perf_counter() # 训练开始计时
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if args.optim_pose==False:
            gaussians.P.requires_grad_(False) # 禁用姿态参数的梯度计算

        # Every 1000 its we increase the levels of SH up to a maximum degree
        if iteration % 1000 == 0:
            gaussians.oneupSHdegree() # 每1000次迭代增加SH级别

        # Pick a random Camera
        if not viewpoint_stack:
            viewpoint_stack = scene.getTrainCameras().copy() # 获取并复制相机数据
        viewpoint_cam = viewpoint_stack.pop(randint(0, len(viewpoint_stack)-1)) # 随机选择一个相机
        pose = gaussians.get_RT(viewpoint_cam.uid) # 获取相机的姿态参数

        # Render
        if (iteration - 1) == debug_from:
            pipe.debug = True # 调试模式开关
        ...
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim(image, gt_image)) # 计算加权损失
        loss.backward()
